...
def a_star_search(state):
    ...
    distance = {}
    parents = {}
    distance[tuple(state)] = 0
    parents[tuple(state)] = tuple(state)
    while len(open_set) > 0:
        n = None
        for v in open_set:
            if n is None or distance[tuple(v)] + score(v) < distance[tuple(n)] + score(n):
                n = v
        if is_final_state(n):
            ...
            for (m, weight) in get_neighbors(list(n)):
                if tuple(m) not in open_set and tuple(m) not in closed_set:
                    ...
                    parents[tuple(m)] = n
                    distance[tuple(m)] = distance[n] + weight
                else:
                    if distance[tuple(m)] > distance[n] + weight:
                        distance[tuple(m)] = distance[n] + weight
                        parents[tuple(m)] = n
